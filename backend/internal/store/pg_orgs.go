...
func (s *PgStore) GetOrgSettings(ctx context.Context, orgID string) (map[string]any, error) {
	...
	var hasWebhook bool
	var forwardingEnabled, autoReplyEnabled, externalForwardingAllowed, agentSendEnabled bool
	...
	err := s.q.QueryRow(ctx,
		`SELECT name, onboarding_completed, (resend_api_key_encrypted IS NOT NULL) as has_key, resend_rps,
		        ...
		        forwarding_enabled, auto_reply_enabled, external_forwarding_allowed, agent_send_enabled
		...
		&forwardingEnabled, &autoReplyEnabled, &externalForwardingAllowed, &agentSendEnabled)
	if err != nil {
		return nil, err
	}
	...
	return map[string]any{
		...
		"agent_send_enabled":   agentSendEnabled,
	}, nil
}
